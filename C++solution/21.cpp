 ...
 *  Each EAN ends with a check digit, just as a UPC does. The technique for calculating
 *  the check digit is also similar:
 ...
 *	Subtracting 1 give 81. The remainder upon dividing by 10 is 1. When the remainder is subtracted
 *	from 9, the result is 8, which matches the last digit of the original code. Your job is to modify
 *	the upc.c program of Section 4.1 so that it calculates the check digit for an EAN.
 *  The user will enter the first 12 digits of the EAN as a single number:
 * 
 *	Enter the first 12 digits of an EAN: 868148426000
 *	Check digit: 8
 */
#include <iostream>
...
int main()
{
	int d{}, i1{}, i2{}, i3{}, i4{}, i5{}, i6{},
		j1{}, j2{}, j3{}, j4{}, j5{}, j6{};

	std::cout << "Enter the first 13 digits of a European Article Number(EAN):  ";
	scanf_s("%1d%1d%1d%1d%1d%1d%1d%1d%1d%1d%1d%1d%1d", &d, &i1, &i2, &i3, &i4, &i5, &i6, &j1, &j2, &j3, &j4, &j5, &j6);

	int firstSum{ i1 + i3 + i5 + j1 + j3 + j5 };
	int secondSum{ d + i2 + i4 + i6 + j2 + j4 + j6 };
